...
def build_archive(repository: Path, output: Path) -> tuple[str, str]:
    ...
    if not config.archive_source:
        raise ValueError(
            ".release.json has no archive section; this repository releases from the tagged tree"
        )
    ...
    with zipfile.ZipFile(
        output, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9, strict_timestamps=True
    ) as archive:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=None, help="archive path; defaults to dist/<archive.name>"
    )
    ...
    output = args.output or (
        config.repository
        / "dist"
        / (config.archive_name or f"{Path(config.archive_source).name}.zip")
    )
