    ...
    for f in config.version_fields:
        ...
        if f.kind == "json":
            ...
            rendered[path] = _replace_one(
                text, f.pattern or "", (f.template or "").format(version=version), f.path
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--timezone", default=None, help="IANA zone; defaults to .release.json timezone"
    )
    ...
    if args.next_from_tags:
        ...
        try:
            ...
            print(
                f"warning: zone {zone} unavailable ({err}); using the local system date. Install tzdata to fix.",
                file=sys.stderr,
            )
