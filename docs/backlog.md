...
- Fire OS 7 (Android 9, API 28) support: guards added 2026-09-11; closed 2026-09-11: Device Owner impossible on Fire OS 7 (protected profile owner in the image, docs/fire-os-assessment.md); only the no-owner "lite" tier could cover Fires. `Wifi.kt` used `addNetworkPrivileged` (API 31) and `setLocationEnabled` (API 30) without version guards; add the legacy `WifiManager.addNetwork` path and guard the location call before qualifying a Fire HD 8 Plus (2020) as Device Owner.
